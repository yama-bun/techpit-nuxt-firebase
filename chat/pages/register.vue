<template>
    <div class="m-4 p-4 bg-white shadow rounded">
        ...
        <form>
            <div class="flex items-center justify-center flex-col w-full h-full mt-8">
                <div class="flex items-center justify-center w-32 h-32 bg-gray-200 rounded-full border border-gray-400">
                    <template v-if="form.imageUrl.val">
                        <img :src="form.imageUrl.val" class="w-32 h-32 object-cover border rouded-full" @click="selectImage">
                    </template>
                    <template v-else>
            ...
            <div class="h-20 mb-6">
                <input type="text" class="block w-full py-3 px-4 appearance-none bg-gray-200 textdarkGray border rounded leading-tight focus:outline-none focus:bg-white">
            </div>
